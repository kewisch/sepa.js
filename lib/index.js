...
class SepaDocument {
  ...
  toString() {
    ...
    const documentDeclaration = `<?xml version="${document.xmlVersion}" encoding="${
      document.xmlEncoding
    }"?>`;
    return documentDeclaration + serializeToString(document);
  }
}
...
class SepaPaymentInfo {
  ...
  validate() {
    ...
    if (this.method === PaymentInfoTypes.DirectDebit) {
      ...
      assertions.assertDate(this.requestedExecutionDate, 'requestedExecutionDate');
    }

    assertions.assertLength(this[`${pullFrom}Name`], null, 70, `${pullFrom}Name`);
    assertions.assertLength(this[`${pullFrom}Street`], null, 70, `${pullFrom}Street`);
    assertions.assertLength(this[`${pullFrom}City`], null, 70, `${pullFrom}City`);
    assertions.assertLength(this[`${pullFrom}Country`], null, 2, `${pullFrom}Country`);
    assertions.assertIban(this[`${pullFrom}IBAN`], `${pullFrom}IBAN`);
    assertions.assertLength(this[`${pullFrom}BIC`], [0, 8, 11], `${pullFrom}BIC`);
...
class SepaTransaction {
  ...
  validate() {
    ...
    assertions.assertLength(this[`${pullFrom}Country`], null, 2, `${pullFrom}Country`);
    assertions.assertIban(this[`${pullFrom}IBAN`], `${pullFrom}IBAN`);
    assertions.assertLength(this.remittanceInfo, null, 140, 'remittanceInfo');
  }
