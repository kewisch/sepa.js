...
var SepaBuilder = require('./sepa.builder');
  ...
  () => {
    ...
    test('Detects valid BIC/IBAN pairs in SepaTransaction and SepaPaymentInfo', () => {
      ...
      debtorCreditorBicIbans.forEach(item => {
        var info = new SepaBuilder.SepaPaymentInfoBuilder()
          .withIban( item.creditor.iban)
          .withBic(item.creditor.bic)
          .build();
        expect(() => info.validate()).not.toThrow();

        var tx = new SepaBuilder.SepaTransactionBuilder(info)
          .withIban(item.debtor.iban)
          .withBic(item.debtor.bic)
          .build();
