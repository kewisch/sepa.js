...
describe('xml generation for direct debit documents', () => {
  ...
  test('rejects amendment with isAmended=true but no details (strict validation)', () => {
    // GIVEN
    var doc = new SEPA.Document(PAIN_FOR_DIRECT_DEBIT);
    doc.grpHdr.created = new Date();
    ...
    tx.amendment = {
      isAmended: true
      // No originalCreditorSchemeId - this should throw
    };
    info.addTransaction(tx);

    // WHEN/THEN - Should throw an error
    expect(() => doc.toXML()).toThrow(/amendment.isAmended is true but no amendment details provided/);
  });
