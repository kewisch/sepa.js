...
function validDirectDebitDocument({
  ...
  painFormat = PAIN_FOR_DIRECT_DEBIT,
}) {
...
describe('XML tests', () => {
  ...
  test('Does not allow for XSS injection in attribute values', () => {
    // GIVEN a malicious value for a node attribute
    // This value attempts to:
    // 1. Break out: The first double quote (") is meant to prematurely close the attribute value.
    // 2. Inject code: The `onmouseover` event handler is then injected.
    // 3. Hide the rest: The final `//` is to comment out the rest of the legitimate attribute value.
    const maliciousInput = '1" onmouseover="alert(\'XSS\')" data-other="//';

    // WHEN we use the value in an xml node and serialize it
    const node = new _SepaXmlNode('Image', '', {
      'src': maliciousInput
    });
    const xmlString = node.toString();

    // THEN the output has escaped all critical characters
    const expectedAttrValue = '1&quot; onmouseover=&quot;alert(&apos;XSS&apos;)&quot; data-other=&quot;//';
    const expectedXmlPart = ` src="${expectedAttrValue}"`;
    expect(xmlString).toBe(`<Image${expectedXmlPart}/>`);
  });

  test('Does not allow for XSS injection in text content', () => {
    // GIVEN a malicious value for xml text content
    // This value attempts to:
    // 1. break out of the tag content with a closing tag: </Element>
    // 2. Injects a script tag: <script>alert("XSS")</script>
    // It also includes all other critical escape characters: & " '
    const maliciousInput = '</Container><script>alert("XSS")</script>&"\'';

    // WHEN we use the value in an xml node and serialize it
    const node = new _SepaXmlNode('Container', maliciousInput);
    const xmlString = node.toString();

    // THEN the output has escaped all critical characters
    const expectedContent = '&lt;/Container&gt;&lt;script&gt;alert(&quot;XSS&quot;)&lt;/script&gt;&amp;&quot;&apos;';
    expect(xmlString).toBe(`<Container>${expectedContent}</Container>`);
  });

  // This attack exploits a lack of ampersand escaping, allowing an attacker to inject
  // pre-encoded HTML entities (&lt; &gt;) that are decoded into live HTML (< >)
  // by a downstream HTML parser.
  test('should double-escape pre-encoded entities in attribute values', () => {
    // GIVEN a malicious value for an xml attribute
    // This value attempts to:
    // 1. Starts with a quote to try and break out of the attribute value with the "
    // 2. Contains an ampersand followed by a malicious entity (&lt;script...)
    const maliciousInput = '"&lt;script&gt;alert(1)&lt;/script&gt;';

    // WHEN we use the value in an xml node and serialize it
    const node = new _SepaXmlNode('Container', '', {
      'data-code': maliciousInput
    });
    const xmlString = node.toString();

    // THEN the output has escaped all critical characters
    const expectedOutput = '<Container data-code="&quot;&amp;lt;script&amp;gt;alert(1)&amp;lt;/script&amp;gt;"/>';
...
describe('structured postal address tests', () => {
  ...
  test.each([
    ...
    ['creditorBuildingName', 'this building name is longer than thirty five characters'],
  ...
  test('emits the PostalAddress24 elements in schema order for pain.008.001.08', () => {
    // GIVEN
    const doc = validDirectDebitDocument({painFormat: 'pain.008.001.08'});
    const tx = doc._paymentInfo[0]._payments[0];
    tx.debtorDepartment = 'Accounting';
    tx.debtorSubDepartment = 'Receivables';
    tx.debtorStreetName = 'Aldersgate Street';
    tx.debtorBuildingNumber = '160';
    tx.debtorBuildingName = 'Barbican';
    tx.debtorFloor = '3';
    tx.debtorPostBox = 'PO 42';
    tx.debtorRoom = '301';
    tx.debtorPostCode = 'EC1A 4HD';
    tx.debtorTownName = 'London';
    tx.debtorTownLocationName = 'City of London';
    tx.debtorDistrictName = 'Greater London';
    tx.debtorCountrySubDivision = 'ENG';
    tx.debtorCountry = 'GB';
    ...
    // THEN
    const select = xpath.useNamespaces({p: 'urn:iso:std:iso:20022:tech:xsd:pain.008.001.08'});
    const address = select('//p:DrctDbtTxInf/p:Dbtr/p:PstlAdr', dom, true);
    expect(childNames(address)).toEqual([
      'Dept', 'SubDept', 'StrtNm', 'BldgNb', 'BldgNm', 'Flr', 'PstBx', 'Room',
      'PstCd', 'TwnNm', 'TwnLctnNm', 'DstrctNm', 'CtrySubDvsn', 'Ctry']);
    expect(select('p:BldgNm', address, true).textContent).toBe('Barbican');
  });

  test.each([
    ['creditorBuildingName', PAIN_FOR_TRANSFERS, 'pain.001.001.03'],
    ['creditorFloor', PAIN_FOR_TRANSFERS, 'pain.001.001.08'],
    ['creditorDepartment', 'pain.001.001.03', 'pain.001.001.02'],
  ])('accepts %s in %s and rejects it in %s', (field, supportedFormat, unsupportedFormat) => {
    // GIVEN
    const supportedDoc = validTransferDocument({painFormat: supportedFormat});
    supportedDoc._paymentInfo[0]._payments[0][field] = 'value';

    const unsupportedDoc = validTransferDocument({painFormat: unsupportedFormat});
    unsupportedDoc._paymentInfo[0]._payments[0][field] = 'value';

    // WHEN THEN
    expect(() => supportedDoc.toString()).not.toThrow();
    expect(() => unsupportedDoc.toString()).toThrow(`${field} is not supported by ${unsupportedFormat}`);
  });
