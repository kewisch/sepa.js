...
function validateSchema(xml, schema) {
  let xmlDoc = XmlDocument.fromString(xml);
  let xsdDoc = XmlDocument.fromBuffer(schema);

  let validator = XsdValidator.fromDoc(xsdDoc);
...
describe('structured postal address schema validation tests',
  () => {
    ...
    test.each([
      'pain.001.001.09',
      'pain.008.001.08',
    ])('%p schema matches with every PostalAddress24 element', (format) => {
      var doc = new SEPA.Document(format);
      doc.grpHdr.id = 'XMPL.20140201.TR0';
      doc.grpHdr.created = new Date();
      doc.grpHdr.initiatorName = 'Example LLC';

      var isTransfer = format.indexOf('pain.001') === 0;
      var party = isTransfer ? 'creditor' : 'debtor';
      var info = doc.createPaymentInfo();
      if (isTransfer) {
        info.requestedExecutionDate = new Date();
        info.debtorIBAN = 'DE87123456781234567890';
      ...
      var tx = info.createTransaction();
      tx[party + 'Name'] = 'Example Customer';
      tx[party + 'IBAN'] = 'DE40987654329876543210';
      tx[party + 'BIC'] = 'CUSTDEM0XXX';
      if (!isTransfer) {
        tx.mandateId = 'XMPL.CUST487.2014';
        tx.mandateSignatureDate = new Date('2014-02-01');
      }
      ...
      tx.remittanceInfo = 'INVOICE 54';
      tx.end2endId = 'XMPL.CUST487.INVOICE.54';
      tx[party + 'Department'] = 'Accounting';
      tx[party + 'SubDepartment'] = 'Receivables';
      tx[party + 'StreetName'] = 'Musterstrasse';
      tx[party + 'BuildingNumber'] = '17';
      tx[party + 'BuildingName'] = 'Tower A';
      tx[party + 'Floor'] = '3';
      tx[party + 'PostBox'] = '42';
      tx[party + 'Room'] = '301';
      tx[party + 'PostCode'] = '80331';
      tx[party + 'TownName'] = 'Munich';
      tx[party + 'TownLocationName'] = 'Altstadt';
      tx[party + 'DistrictName'] = 'Oberbayern';
      tx[party + 'CountrySubDivision'] = 'BY';
      tx[party + 'Country'] = 'DE';
      info.addTransaction(tx);
