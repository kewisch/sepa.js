...
xdescribe('#assertSepaIdSet1', () => {
  it('should throw when Sepa ID is not valid', () => {
    expect(() => assertSepaIdSet1('not valid', 'test')).toThrow();
...
describe.only('#assertSepaIdSet2', () => {
  it.only('should throw when SEPA ID is not valid', () => {
    expect(() => assertSepaIdSet2('not valid', 'test')).toThrow();
  });

  it('should not throw when SEPA is valid - without space', () => {
    expect(() => assertSepaIdSet2('XMPL.CUST487.INVOICE.54', 'test')).not.toThrow();
  });
});