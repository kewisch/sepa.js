 ...
 * SEPA.Transaction            -- class for generic transactions (defaults to DirectDebit so we don't break the API)
 * SEPA.DirectDebitTransaction -- class for generic transactions already bound to DirectDebit type
 * SEPA.TransferTransaction    -- class for generic transactions already bound to Transfer type
 * SEPA.GenericTransaction     -- class for generic transactions
...
(function(exports) {
  ...
  var SEPATypes = {
    DirectDebit: 'CstmrDrctDbtInitn',
    Transfer:    'CstmrCdtTrfInitn'
  };

  function SepaDocument(optionsOrPainFormat, type) {
    var painFormat;
    if (optionsOrPainFormat) {
      painFormat = typeof optionsOrPainFormat === 'string' ? optionsOrPainFormat : optionsOrPainFormat.painFormat;
      type = typeof optionsOrPainFormat === 'string' ? type : optionsOrPainFormat.type;
    }

    this._type = type || SEPATypes.DirectDebit;
    this._paymentInfo = [];
    this.painFormat = painFormat || (this._type === SEPATypes.DirectDebit ? 'pain.008.003.02' : 'pain.001.001.02');
  ...
  SepaDocument.prototype = {
    ...
    /** SEPA Document type setting, determines the root element */
    _type: null,
    ...
    toXML: function() {
      ...
      body.setAttributeNS(XSI_NAMESPACE, 'xmlns', XSI_NS + this.painFormat);
      body.setAttributeNS(XSI_NAMESPACE, 'xmlns:xsi', XSI_NAMESPACE);
      body.setAttributeNS(XSI_NAMESPACE, 'xsi:schemaLocation', XSI_NS + this.painFormat + ' ' + this.painFormat + '.xsd');
   ...
   */
  function SepaPaymentInfo(method) {
    this.method = method || PaymentInfoTypes.DirectDebit;
    this._payments = [];
  }
  ...
  SepaPaymentInfo.prototype = {
    ...
    addTransaction: function(pmt) {
      if (!(pmt instanceof SepaGenericTransaction)) {
        throw new Error('Given Transaction is not member of the SepaGenericTransaction class');
      }
    ...
    toXML: function(doc) {
      ...
      r(pmtInf, 'BtchBookg', this.method === PaymentInfoTypes.DirectDebit ? this.batchBooking : false);
      ...
      var creditorScheme = n(pmtInf, emitterNodeName + 'SchmeId', 'Id', 'PrvtId', 'Othr');
      r(creditorScheme, 'Id', this[pullFrom + 'Id']);
  ...
  function SepaGenericTransaction(type) {
    this._type = type || TransactionTypes.DirectDebit;
  }

  SepaGenericTransaction.TransactionTypes = TransactionTypes;

  SepaGenericTransaction.prototype = {
    ...
    toXML: function(doc) {
      ...
      r(txInf, 'InstrId', this.id); // FIXME this does't validate
  ...
  exports.Transaction            = SepaGenericTransaction; // For non-breaking change
  exports.DirectDebitTransaction = SepaGenericTransaction.bind(undefined, TransactionTypes.DirectDebit);
  exports.TransferTransaction    = SepaGenericTransaction.bind(undefined, TransactionTypes.TransferTransaction);
  exports.GenericTransaction     = SepaGenericTransaction;
