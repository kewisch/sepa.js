...
(function(exports) {
  ...
  SepaTransaction.prototype = {
    ...
    validate: function() {
      ...
      if (this.amendment) {
        if (typeof this.amendment.isAmended !== 'boolean') {
          throw new Error('amendment.isAmended must be a boolean');
        }
    ...
    toXML: function(doc) {
      ...
      if (this._type === TransactionTypes.DirectDebit) {
        ...
        if (this.amendment && this.amendment.isAmended) {
          r(mandate, 'AmdmntInd', 'true');
          
          // Check if we have any amendment details to include
          var hasDetails = !!(this.amendment.originalCreditorSchemeId);
          
          if (hasDetails) {
            var amendmentDetails = n(mandate, 'AmdmntInfDtls');
            
            // Add original creditor scheme ID if provided (for ICS migration)
            if (this.amendment.originalCreditorSchemeId) {
              var originalCreditorScheme = n(amendmentDetails, 'OrgnlCdtrSchmeId', 'Id', 'PrvtId', 'Othr');
              r(originalCreditorScheme, 'Id', this.amendment.originalCreditorSchemeId);
              r(originalCreditorScheme, 'SchmeNm', 'Prtry', 'SEPA');
            }
          } else {
            // Strict approach: require at least one amendment detail when isAmended=true
            throw new Error('amendment.isAmended is true but no amendment details provided. Please specify at least originalCreditorSchemeId.');
          }
        } else {
