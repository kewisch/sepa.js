...
/**
 * Regular expression to match any character that is *NOT* a valid XML 1.0 character.
 * 0x09, 0x0A, 0x0D (Tab, LF, CR)
 * 0x20 to 0xD7FF
 * 0xE000 to 0xFFFD
 * 0x10000 to 0x10FFFF
 */
const _INVALID_XML_CHAR_REGEX = /[^\t\n\r\u0020-\uD7FF\uE000-\uFFFD\u{10000}-\u{10FFFF}]/u;
...
function _isValidXmlString(text) {
  return !_INVALID_XML_CHAR_REGEX.test(text);
}
...
function _isValidXmlName(str) {
  ...
  // xml allows letters, digits and -._: as characters for identifiers
  // . and - are not allowed for the first character
  if (!/^[a-zA-Z_:][a-zA-Z0-9\-._:]*$/.test(str)) {
    return false;
  }

  // 'xml' is a reserved prefix. We allow 'xmlns' though because our implementation has a naive namespace implementation
...
export class _SepaXmlNode {
   ...
   * Use the getter and setter to interact with it.
   * @type {string}
   * @private
   */
  _textContent;
  ...
  toString(indentation = 0, prettyPrint = false) {
    ...
    const hasText = this.textContent !== '';
    ...
    if (!hasChildren && hasText) {
      return indent + openingTag + _escapeStringForXml(this.textContent) + closingTag + newline;
    }
 ...
 * @constructor
 */
export function Document(painFormat = DEFAULT_PAIN_FORMAT) {
...
function _addNestedChildren(parent, childrenNames, valueForLastChild = null) {
  ...
  if (valueForLastChild) {
    child.textContent = valueForLastChild;
  }
  return child;
}
...
const POSTAL_ADDRESS_6_AND_24_FORMATS = ['pain.001.001.03', 'pain.001.001.08', 'pain.001.001.09', 'pain.008.001.02', 'pain.008.001.08'];
const POSTAL_ADDRESS_24_FORMATS = ['pain.001.001.09', 'pain.008.001.08'];

/**
 * Structured postal address elements in the ISO 20022 sequence order, with the
 * field suffix read from the party, the schema max length and, when the element
 * is not defined by every postal address type, the formats supporting it.
 * @private
 */
const POSTAL_ADDRESS_ELEMENTS = [
  {element: 'Dept', field: 'Department', maxLength: 70, formats: POSTAL_ADDRESS_6_AND_24_FORMATS},
  {element: 'SubDept', field: 'SubDepartment', maxLength: 70, formats: POSTAL_ADDRESS_6_AND_24_FORMATS},
  {element: 'StrtNm', field: 'StreetName', maxLength: 70},
  {element: 'BldgNb', field: 'BuildingNumber', maxLength: 16},
  {element: 'BldgNm', field: 'BuildingName', maxLength: 35, formats: POSTAL_ADDRESS_24_FORMATS},
  {element: 'Flr', field: 'Floor', maxLength: 70, formats: POSTAL_ADDRESS_24_FORMATS},
  {element: 'PstBx', field: 'PostBox', maxLength: 16, formats: POSTAL_ADDRESS_24_FORMATS},
  {element: 'Room', field: 'Room', maxLength: 70, formats: POSTAL_ADDRESS_24_FORMATS},
  {element: 'PstCd', field: 'PostCode', maxLength: 16},
  {element: 'TwnNm', field: 'TownName', maxLength: 35},
  {element: 'TwnLctnNm', field: 'TownLocationName', maxLength: 35, formats: POSTAL_ADDRESS_24_FORMATS},
  {element: 'DstrctNm', field: 'DistrictName', maxLength: 35, formats: POSTAL_ADDRESS_24_FORMATS},
  {element: 'CtrySubDvsn', field: 'CountrySubDivision', maxLength: 35}
];

/**
 * Add a <PstlAdr> element for the given party, if any address data is set.
 *
 * Structured ISO 20022 elements and address lines take precedence and are
...
function _addPostalAddress(parent, source, prefix) {
  const structuredElements = POSTAL_ADDRESS_ELEMENTS
    .map(({element, field}) => [element, source[prefix + field]]);
 ...
 *
 * Element lengths are identical across all supported pain formats. The
 * formats differ in the elements they define (see POSTAL_ADDRESS_ELEMENTS) and
 * in the number of address lines, which PostalAddress1 (pain.001.001.02) caps
 * at 5 instead of 7.
 *
 * @param source {object}         The payment info or transaction to read from.
 * @param prefix {string}         Field prefix, either 'creditor' or 'debtor'.
...
function validate_postal_address(source, prefix) {
  ...
  for (const {field, maxLength, formats} of POSTAL_ADDRESS_ELEMENTS) {
    assert_length(source[prefix + field], null, maxLength, prefix + field);
    assert(!source[prefix + field] || !formats || formats.includes(source._painFormat),
      prefix + field + ' is not supported by ' + source._painFormat);
  }
...
SepaPaymentInfo.prototype = {
  ...
  /** PostalAddress24 elements, only supported by pain.001.001.09 and pain.008.001.08 (Dept and SubDept also by PostalAddress6 formats) */
  creditorDepartment: null,
  creditorSubDepartment: null,
  creditorBuildingName: null,
  creditorFloor: null,
  creditorPostBox: null,
  creditorRoom: null,
  creditorTownLocationName: null,
  creditorDistrictName: null,
  ...
  /** PostalAddress24 elements, only supported by pain.001.001.09 and pain.008.001.08 (Dept and SubDept also by PostalAddress6 formats) */
  debtorDepartment: null,
  debtorSubDepartment: null,
  debtorBuildingName: null,
  debtorFloor: null,
  debtorPostBox: null,
  debtorRoom: null,
  debtorTownLocationName: null,
  debtorDistrictName: null,
