...
(function(exports) {
  ...
  SepaGroupHeader.prototype = {
    ...
    toXML: function(doc) {
      ...
      var containerNode = createXMLHelper(doc, true, false);
      // Function to create a node even if no value is present
      var requiredNode = createXMLHelper(doc, true, true);
      ...
      var initgPty = containerNode(grpHdr, 'InitgPty');
  ...
  SepaPaymentInfo.prototype = {
    ...
    validate: function() {
      ...
      assert_fixed(this.localInstrumentation, ['CORE', 'COR1', 'B2B', 'SDCL', 'ONCL'], 'localInstrumentation');
    ...
    toXML: function(doc) {
      ...
      var containerNode = createXMLHelper(doc, true, false);
      var optionalNode = createXMLHelper(doc, false, true);
      var requiredNode = createXMLHelper(doc, true, true);
      ...
      var pmtTpInf = containerNode(pmtInf, 'PmtTpInf');
      var pullFrom = this.method === PaymentInfoTypes.DirectDebit ? 'creditor' : 'debtor';
      
      requiredNode(pmtTpInf, 'SvcLvl', 'Cd', 'SEPA');
      optionalNode(pmtTpInf, 'CtgyPurp', 'Cd', this[pullFrom + 'CategoryPurpose']);

      if (this.method === PaymentInfoTypes.DirectDebit) {
        requiredNode(pmtTpInf, 'LclInstrm', 'Cd', this.localInstrumentation);
        requiredNode(pmtTpInf, 'SeqTp', this.sequenceType);
        requiredNode(pmtInf, 'ReqdColltnDt', this.collectionDate.toISOString().substr(0, 10));
      }
      else {
        requiredNode(pmtInf, 'ReqdExctnDt', this.requestedExecutionDate.toISOString().substr(0, 10));
      }

      var emitterNodeName = this.method === PaymentInfoTypes.DirectDebit ? 'Cdtr' : 'Dbtr';
      var emitter = containerNode(pmtInf, emitterNodeName);

      requiredNode(emitter, 'Nm', this[pullFrom + 'Name']);
      if (this[pullFrom + 'Street'] && this[pullFrom + 'City'] && this[pullFrom + 'Country']) {
        var pstl = containerNode(emitter, 'PstlAdr');
      ...
      requiredNode(pmtInf, emitterNodeName + 'Acct', 'Id', 'IBAN', this[pullFrom + 'IBAN']);
      if (this[pullFrom + 'BIC']) {
        var finInstnId = containerNode(pmtInf, emitterNodeName + 'Agt', 'FinInstnId');
        requiredNode(finInstnId, 'BIC', this[pullFrom + 'BIC']);
        optionalNode(finInstnId, 'PstlAdr', 'Ctry', this[pullFrom + 'Country']);
      } else {
        requiredNode(pmtInf, emitterNodeName + 'Agt', 'FinInstnId', 'Othr', 'Id', 'NOTPROVIDED');
      }

      requiredNode(pmtInf, 'ChrgBr', 'SLEV');

      if (this.method === PaymentInfoTypes.DirectDebit) {
        var creditorScheme = containerNode(pmtInf, 'CdtrSchmeId', 'Id', 'PrvtId', 'Othr');
  ...
  SepaTransaction.prototype = {
    ...
    toXML: function(doc) {
      ...
      var paymentId = containerNode(txInf, 'PmtId');
      ...
      if (this._type === TransactionTypes.DirectDebit) {
        requiredNode(txInf, 'InstdAmt', this.amount.toFixed(2)).setAttribute('Ccy', this.currency);

        var mandate = containerNode(txInf, 'DrctDbtTx', 'MndtRltdInf');
        requiredNode(mandate, 'MndtId', this.mandateId);
        requiredNode(mandate, 'DtOfSgntr', this.mandateSignatureDate.toISOString().substr(0, 10));
        ...
        requiredNode(txInf, 'Amt', 'InstdAmt', this.amount.toFixed(2)).setAttribute('Ccy', this.currency);
      }

      if (this[pullFrom + 'BIC']) {
        var finInstnId = containerNode(txInf, recieverNodeName + 'Agt', 'FinInstnId');
        requiredNode(finInstnId, 'BIC', this[pullFrom + 'BIC']);
        optionalNode(finInstnId, 'PstlAdr', 'Ctry', this[pullFrom + 'Country']);
      } else {
        requiredNode(txInf, recieverNodeName + 'Agt', 'FinInstnId', 'Othr', 'Id', 'NOTPROVIDED');
      }

      var reciever = containerNode(txInf, recieverNodeName);
      requiredNode(reciever, 'Nm', this[pullFrom + 'Name']);

      if (this[pullFrom + 'Street'] && this[pullFrom + 'City'] && this[pullFrom + 'Country']) {
        var pstl = containerNode(reciever, 'PstlAdr');
      ...
      requiredNode(txInf, recieverNodeName + 'Acct', 'Id', 'IBAN', this[pullFrom + 'IBAN']);
